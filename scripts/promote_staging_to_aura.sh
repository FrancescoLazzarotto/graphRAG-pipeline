...
#
#   bash scripts/promote_staging_to_aura.sh          # dry run: prints the plan
#   CONFIRM=yes bash scripts/promote_staging_to_aura.sh
set -euo pipefail
...
conda run --no-capture-output -n graphllm python scripts/kg_backup.py \
...
echo "=== 3/6 wipe Aura ==="
conda run --no-capture-output -n graphllm python scripts/kg_wipe.py --yes
...
conda run --no-capture-output -n graphllm python scripts/kg_restore.py \
  --backup-dir "$STAGING_DUMP" \
  --uri "$AURA_URL" --user "$AURA_USER" --password "$AURA_PASSWORD" \
  --database "$AURA_DB"

echo "=== 5/6 rebuild the vector index on Aura ==="
conda run --no-capture-output -n graphllm python scripts/kg_vector_index.py --context-chars 300

echo "=== 6/6 guard ==="
conda run --no-capture-output -n graphllm python scripts/check_vector_index.py --min-resolving 1000
